...
3. **Legacy fallback files** (deprecated, for backward compatibility only):
    - `.clinerules`
    - `.kilocoderules`
...
2. If that doesn't exist or is empty, it falls back to `.kilocoderules-${mode}` file (deprecated only, `.clinerules-${mode}` is not supported)

Mode-specific rules can be configured at both the global and project levels.
...
1. **Check rule locations**: Ensure that your rules are located in supported locations:
    ...
    - Legacy files (deprecated): `.clinerules` or `.kilocoderules`
